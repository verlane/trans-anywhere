...
class DaumDic {
  JapaneseToKorean(keyword) {
    ...
    Return this._GetDaumDic(keyword, wordId, "KUMSUNG_JK")
  }
  ...
  GetDaumEngDic(keyword) {
    ...
    Return this._GetDaumDic(keyword, wordId, "KUMSUNG_EK")
  }

  _GetDaumDic(keyword, wordId, subType) {
    ...
    Loop % fold_exes.length
    {
      ...
      Loop % fold_ex.children.length
      {
        ...
        if (child.tagName == "UL") {
          childText := " " this._RemoveCommonChar(child.querySelectorAll("li p")[0].innerText) "`n"
          childText := childText " " this._RemoveCommonChar(child.querySelectorAll("li p")[1].innerText)
        } else {
          childText := this._RemoveCommonChar(child.innerText)
    ...
    return {simpleData: text, rawData: "", pronFilePath: ""}
  }

  _RemoveCommonChar(arg) {
